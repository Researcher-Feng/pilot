    ...
    def initialize_agents(self):
        ...
        if self.config.agent.get("use_solution_tree", False):
            ...
            expert_model_config = ModelConfig(
                model_type=self.config.model.model_type_teacher,  # 使用教师模型配置
                model_name=self.config.model.api_name_student,
                base_url=self.config.model.base_url_student if hasattr(self.config.model, 'base_url_student') else None,
                temperature=0.3,  # 学霸需要更确定性
                max_tokens=2000
            )
            self.expert_agent.model_init(expert_model_config)
            self.expert_agent.config_create("thread_id", "expert_1")
            self.expert_agent.agent_init(
                expert_model_config,
                prompt_sys_name=EXPERT_STUDENT_PROMPT,
                tools_list=[]
            )
            logger.info(f"✅ Expert student agent initialized: {self.config.model.api_name_student}")
        ...
        if self.config.agent.get("use_solution_tree", False):
            teacher_kwargs["prompt_sys_name"] = TEACHER_WITH_TREE_PROMPT
    ...
    def _setup_cognitive_state(self):
        ...
        if self.config.agent.get("use_cognitive_state", False):
            cognitive_state = StudentCognitiveState(
                ...
                learning_style=self.config.agent.get("learning_style", "visual")
            )
        ...
        try:
            ...
            if summary_model_type == "local":
                ...
                self.summary_llm = ChatOllama(
                    ...
                    temperature=0.1,  # 摘要需要确定性
                    num_predict=300  # 摘要不需要太长
                )
...
def solve_with_dialogue(config, logger, val_dataset, multi_agent_system):
    ...
    for i, (prompt, label_ans) in tqdm(
        ...
        if config.agent.get("use_solution_tree", False):
            solution_tree = multi_agent_system.generate_solution_tree(prompt)
            if solution_tree:
        ...
        # 计算一题多解分数（如果启用解题树）
        if config.agent.get("use_solution_tree", False) and config.agent.get("evaluate_multi_solution", False):
...
if __name__ == '__main__':
    ...
    -----------------------------------
    '''
    # overrides.append('data.val_files=D:\DeepLearning\Code\LangChain\dataset/GSM8k_test_with_prompt3.parquet')
    overrides.append('data.val_files=/mnt/t2-6tb/medical/SocraticLM_langchain/LangChain_3090/dataset/GSM8k_test_with_prompt3.parquet')
    ...
    overrides.append('+model.temperature_teacher=0.7')
    overrides.append('+model.max_tokens_teacher=2000')
    overrides.append('+agent.max_samples=20')    # 测试用样本数量
    overrides.append('+agent.debug_samples=2')   # 查看完整response
    ...
    overrides.append('+model.api_name_student=qwen3:4b-tuned-4k')  # qwen3-4b-4k:latest   qwen3:4b-tuned-4k
    ...
    # overrides.append('+model.api_name_teacher=qwen3:4b-tuned-8k')  # qwen3-4b-4k:latest   qwen3:4b-tuned-4k
    ...
    overrides.append('+agent.carelessness_level=6')                    # 粗心程度 (1-10)
    overrides.append('+agent.math_background=intermediate')            # 数学背景
    overrides.append('+agent.response_style=thoughtful')               # 回复风格
    overrides.append('+agent.preferred_method=algebraic')              # 偏好方法
    overrides.append('+agent.learning_style=visual')                   # 学习风格
