...
class SimpleLocalAgent:
    ...
    def invoke(self, input_dict, config=None, context=None):
        ...
        if self.debug_mode:
            print("\n" + "=" * 80)
            print(f"🔍 DEBUG - {self.agent_type.upper()} AGENT FULL PROMPT:")
            print("=" * 80)
            print(full_prompt)
            print("=" * 80 + "\n")
...
class SolutionTree:
    ...
    def _detect_student_method(self):
        """检测学生使用的方法"""
        ...
        elif any(word in content for word in ["logic", "reason", "therefore", "because"]):
            return "logical"
        else:
            return "unknown"
...
class SimpleAgent(object):
    ...
    def _build_full_prompt(self, user_input, memory_context):
        ...
        # 系统提示词
        if self.prompt_sys:
            prompt_parts.append(f"System: {self.prompt_sys}")
    ...
    def _build_solution_tree_context(self):
        ...
        if self.agent_type == "teacher":
            ...
            if comparison["closest_expert_path"]:
                context_parts.append(
                    f"Expert solution available using {comparison['closest_expert_path']['method']} method")
                context_parts.append(f"Similarity to student approach: {comparison['similarity']:.2f}")

        elif self.agent_type == "student":
            # 学生看到自己的进度
            if self.solution_tree.current_student_path:
                context_parts.append(
    ...
    def complete_student_solution(self, success, final_answer=None):
        ...
        if (self.agent_type == "student" and self.use_solution_tree and
            ...
            return self.solution_tree.complete_student_path(success, final_answer)
        return None
    ...
    def agent_init(self, model_config, prompt_sys_name=None, tools_list=None, context_schema=Context,
        ...
        self.math_background_level = kwargs.get("math_background", False)
        ...
        if model_config.model_type == "local":
            ...
            if tools_list:
                self.local_agent.add_tools(tools_list)
            if self.context:
    ...
    def _adjust_prompt_based_on_settings(self):
        ...
        if self.agent_type == "student":
            ...
            elif self.math_background_level == "intermediate":
    ...
    def _invoke_local_with_memory(self, input_text):
        ...
        # 构建完整提示词（包含内存上下文）
        full_input = self._build_input_with_memory(input_text, memory_context)
...
class ExpertStudentAgent(SimpleAgent):
    ...
    def generate_solution_tree(self, problem):
        """生成解题树"""
        prompt = f"""Generate multiple solution paths for this math problem:

Problem: {problem}

Please provide at least 2-3 different approaches and structure your response using the SolutionTree format."""
        ...
        try:
            ...
            if "<SolutionTree>" in response:
                ...
                for block in path_blocks:
                    if "<Path" in block:
                        # 提取路径信息
                        method = self._extract_xml_tag(block, "method")
                        complexity = self._extract_xml_tag(block, "complexity")
                        innovation = self._extract_xml_tag(block, "innovation")
