...
from v9.prompt.dialogue_cognitive import STUDENT_COGNITIVE_STATE_PROMPT, STUDENT_COGNITIVE_STATE_PROMPT_2
...
class StudentCognitiveState:
    ...
    def __init__(self,
        ...
        self.error_patterns = []
        self.method_preferences = [self.preferred_method]
    ...
    def update_based_on_interaction(self, problem, student_approach, errors, method_used, success):
        ...
        # 更新方法偏好
        if method_used:
            self.method_preferences.append(method_used) if method_used not in self.method_preferences else None

        # 更新数学背景（基于成功率和方法复杂度）
        if success and len(student_approach) > 3:  # dddd
    ...
    def get_prompt_context(self):
        """获取提示词上下文"""
        # return STUDENT_COGNITIVE_STATE_PROMPT_EASY
