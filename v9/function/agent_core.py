...
class SimpleAgent(object):
    ...
    def __init__(self, agent_type: str = "student", debug_mode: bool = False):
        ...
        self.is_first_student_response = True
        ...
        self.solution_tree_gold_section = None
    ...
    def set_solution_tree(self, solution_tree: SolutionTree, solution_tree_section: str):
        """设置解题树"""
        self.solution_tree = solution_tree
        self.solution_tree_gold_section = solution_tree_section
    ...
    def trigger_parallel_thinking(self, impasse_detected: bool, N: int = 3, raw_problem=None):
        ...
        branch_from = self.solution_tree.current_student_path[-1] if self.solution_tree.current_student_path else self.solution_tree.root_node_id
    ...
    def _build_full_prompt(self, user_input, memory_context, prompt_type='api'):
        ...
        if self.agent_type == "student":
            if prompt_type == 'local':
                ...
                if self.use_solution_tree and self.solution_tree:
                    tree_summary = self.solution_tree.visualize()[:500]  # Truncate for token limit
                    if messages[-1]['role'] == 'user':
                        messages[-1]['content'] += f"\n\nCurrent Solution Tree Summary:\n{tree_summary}\nExtend or verify branches as needed."
    ...
    def record_student_step(self, step_content, method_used=None, raw_problem=None):
        """记录学生解题步骤"""
        if self.use_solution_tree and self.solution_tree:
            ...
            branch_from = None if self.is_first_student_response else self.solution_tree.current_student_path[-1] if self.solution_tree.current_student_path else None
            self.solution_tree.parse_and_add_student_response(step_content, method_used, model=refine_model, add_as_branch_from=branch_from,
                                                              raw_problem=raw_problem, solution_tree_gold_section=self.solution_tree_gold_section)
        ...
        self.trigger_parallel_thinking(impasse, raw_problem=raw_problem)
    ...
    def enable_conversation_summary(self, summary_config: SummaryConfig, summary_llm=None, summary_mode='batch'):
        """启用对话摘要功能"""
        ...
        logger.info(f"✅ Conversation summary {status} for {self.agent_type} agent (Mode: {summary_mode})")
    ...
    def multi_agent_chat_explicit(self, teacher_agent, problem: str, raw_problem: str, correct_answer: str,
        ...
        self.problem_statement = raw_problem
        if kwargs.get("solution_tree", False):
            self.solution_tree.set_gold_tree(kwargs['solution_tree'].solution_paths)
        """模式1: 显式交互 - 教师和学生直接对话"""
        ...
        if hasattr(self, 'memory') and self.memory and getattr(self.memory, 'enabled', False):
            ...
            global_memory.add_message('teacher', problem, first_msg=True)
            if dialogue_record.debug_mode:
        ...
        if hasattr(teacher_agent, 'memory') and teacher_agent.memory and getattr(teacher_agent.memory, 'enabled', False):
            ...
            teacher_memory.add_message('teacher', problem, first_msg=True)
            if dialogue_record.debug_mode:
        ...
        try:
            ...
            # 记录学生第一步解题步骤
            if self.use_solution_tree and self.solution_tree:
                self.record_student_step(student_response, method_used=self._detect_method_from_response(student_response), raw_problem=raw_problem)
                if dialogue_record.debug_mode:
            ...
            for turn in range(self.max_turns):
                ...
                # 教师回应
                analysis = None
                if self.use_solution_tree:  # dddddddd
                ...
                if self.parallel_thinking_enabled and turn == 0:
                    ...
                    teacher_input = teacher_agent._format_teacher_input(self.dialogue_history, prompt_type=teacher_prompt_type, analysis=analysis)
                ...
                self._add_to_memory(global_memory, teacher_memory, 'teacher', teacher_response,
                                    dialogue_record.debug_mode)
                self._check_and_generate_summary(global_memory, teacher_memory, dialogue_record.debug_mode)
                ...
                # 学生回应
                student_prompt_type = 'local' if (
                            hasattr(self, 'local_agent') and self.local_agent) else 'api'
    ...
    def _format_teacher_input(self, history, prompt_type='api', analysis=None):
        """格式化教师输入，支持本地和API调用"""
        if prompt_type == 'api':
            messages = []
            system_prompt = self.get_prompt_str(self.prompt_sys)

            if self.memory and self.memory.enabled:
                if self.memory.current_summary:
                    messages.append \
                        ({"role": "user", "content": f"Previous Conversation Summary:\n{self.memory.current_summary}"})

                format_history = self.memory.conversation_history
            else:
                format_history = history

            for msg in format_history:
                if isinstance(msg, dict):
                    role = msg["role"]
                    content = msg["content"]
                else:
                    role, content = msg
                msg_role = "assistant" if role == self.agent_type else "user"
            ...
            elif messages and messages[0]['role'] == 'assistant':
            ...
            if self.use_solution_tree and self.solution_tree:
                if messages[-1]['role'] == 'user':
                    messages[-1]['content'] += f"\n\nStudent Tree Analysis: {analysis}\nBased on the analysis, provide targeted guidance."
    ...
    def _format_student_input(self, problem, history, prompt_type='api'):
        ...
        if prompt_type == 'api':
            ...
            if messages and messages[-1]['role'] == 'user' and len(messages) != 1:
...
class ExpertStudentAgent(SimpleAgent):
    ...
    def generate_solution_tree(self, problem, answer):
        """生成解题树"""
        prompt = TREE_GENERATE_PROMPT.format(problem=problem, answer=answer)
        prompt = [{"content": prompt, "role": "user"}]
        response = self._invoke_agent(prompt)
        return self._parse_solution_tree(response, problem)  # Assume parsing handles weights
