...
from v7.utils.evaluator import logger, extract_answer
from v7.function.api_client import create_api_client, RawAPIClient, RawOllamaClient
...
from v7.function.middleware import CustomMiddleware, ModelConfig, MiddlewareFunc
from v7.function.contex_fun import Context
from v7.function.format_fun import ResponseFormat
from v7.function.record_eval import DialogueRecord
from v7.prompt.system import *
from v7.prompt.dialogue_tree_parallel import *
from v7.prompt.dialogue_socratic import *
...
class SimpleAgent(object):
    ...
    def _build_full_prompt(self, user_input, memory_context, prompt_type='api'):
        ...
        if self.agent_type == "student":
            if prompt_type == 'local':
                ...
                # 1. System prompt (if exists)  # dddd
                if self.prompt_sys:
                    # 构建系统消息内容，包含所有上下文信息
    ...
    def _create_simple_agent(self):
        ...
        class SimpleAgentWrapper:
            ...
            def invoke(self, input_dict, config=None, context=None):
                try:
                    api_messages = [{"role": "system", "content": self.prompt_sys}]
                    # 提取用户输入
                    messages = input_dict.get("messages", [])
                    for msg in messages:
    ...
    def _format_student_input(self, problem, history, prompt_type='api'):
        ...
        if prompt_type == 'api':
            ...
            if history:
                ...
                if messages[-1]['role'] == 'user':
                    messages[-1]['content'] += "\n\n" + STUDENT_RESPONSE_STUDYING_SOLVING.format(question=problem)
                if self.parallel_thinking_enabled:
