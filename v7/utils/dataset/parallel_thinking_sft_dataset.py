...
class RawDataset(Dataset):
    def __init__(self, parquet_file: Union[str, List[str]], config):
        prompt_key = config.get("prompt_key", "prompt")
        prompt_dict_keys = config.get("prompt_dict_keys", None)
        response_key = config.get("response_key", "response")
        raw_problem_key = config.get("raw_problem_key", "response")
        response_dict_keys = config.get("response_dict_keys", None)
        raw_problem_dict_keys = config.get("raw_problem_dict_keys", None)
        ...
        self.first_prompt = config.get('first_prompt', None)
        self.parquet_file = parquet_file
        self.prompt_key = prompt_key if isinstance(prompt_key, (tuple, list)) else [prompt_key]
        self.response_key = response_key if isinstance(response_key, (tuple, list)) else [response_key]
        self.raw_problem_key = raw_problem_key if isinstance(raw_problem_key, (tuple, list)) else [raw_problem_key]
        self.raw_problem_dict_keys = raw_problem_dict_keys if raw_problem_dict_keys else []
    ...
    def _read_files_and_tokenize(self):
        ...
        if self.first_prompt == 'raw':
            self.prompts = self.dataframe['question']
        ...
        self.raw_problem = self.dataframe[self.response_key]
        for key in self.raw_problem_dict_keys:
            try:
                self.raw_problem = self.raw_problem.apply(lambda x: series_to_item(x)[key], axis=1)  # noqa: B023
            except Exception as e:
                print(f"self.raw_problem={self.raw_problem}")
                raise
        if isinstance(self.raw_problem, pd.DataFrame):
            self.raw_problem = self.raw_problem.squeeze()
        self.raw_problem = self.raw_problem.tolist()
...
class ParallelThinkingSFTDataset(Dataset):
    ...
    def __init__(self, parquet_files: Union[str, List[str]], tokenizer, config):
        ...
        first_prompt = config.get('first_prompt', None)
    ...
    def compute_structured_position_ids(self, response_ids: torch.Tensor) -> torch.Tensor:
        ...
        while i < seq_len:
            ...
            if tok == self.start_parallel_token:
                ...
                # Collect path blocks
                while temp_i < seq_len:  # 确保temp_i始终小于seq_len
                    ...
                    if response_ids[temp_i].item() == self.start_path_token:
                        ...
                        # 处理单个path块，确保不越界
                        while temp_i < seq_len and nonpad_mask[temp_i]:
                            pos_ids[temp_i] = local_pos
                            current_token = response_ids[temp_i].item()
                            if current_token == self.end_path_token:
                                break
                            local_pos += 1
                            temp_i += 1
                        else:
                            # 如果因为到达序列末尾而退出循环，需要调整temp_i
                            temp_i = min(temp_i, seq_len - 1)

                        # 关键修复：确保path_end不越界
                        path_end = min(temp_i, seq_len - 1)
                        # 添加安全检查
                        if path_end < seq_len:
                ...
                # 更新主索引，确保不越界
                i = min(temp_i, seq_len - 1)
                max_path_len = max(path_lengths) if path_lengths else 0

                # 处理结束标签，确保i不越界
                if i < seq_len and nonpad_mask[i] and response_ids[i].item() == self.end_parallel_token:
                    pos_ids[i] = curr_pos + max_path_len + 1
                    curr_pos += max_path_len + 2  # +1 for start_parallel, +1 for end_parallel
                    i += 1
                else:
                    curr_pos += max_path_len + 1 if max_path_len > 0 else 1
            else:
                pos_ids[i] = curr_pos
                curr_pos += 1
                i += 1
...
# class CustomParallelThinkingSFTDataset(ParallelThinkingSFTDataset):
#     def __getitem__(self, item):
#         tokenizer = self.tokenizer
#
#         prompt = self.prompts[item]
...
#         # attention_mask_1d = torch.cat((prompt_attention_mask, response_attention_mask), dim=-1)
#         attention_mask_1d = prompt_attention_mask
#
#         # padding to max length
#         sequence_length = input_ids.shape[0]
#         if sequence_length < self.max_length:
...
#             padded_attention_mask_1d = torch.zeros(size=(self.max_length - sequence_length,),
#                                                    dtype=attention_mask_1d.dtype)
#             input_ids = torch.cat((input_ids, padded_input_ids))
#             attention_mask_1d = torch.cat((attention_mask_1d, padded_attention_mask_1d))
#         elif sequence_length > self.max_length:
#             if self.truncation == "left":
...
class CustomParallelThinkingSFTDataset(ParallelThinkingSFTDataset):
    def __getitem__(self, item):
        ...
        # Assuming self.df is a pandas DataFrame from Parquet with 'extra_info' column
        row = self.dataframe.iloc[item]
        extra_info = row['extra_info']  # Could be a dict or JSON string
        if isinstance(extra_info, str):
            try:
                extra_info = json.loads(extra_info)
            except json.JSONDecodeError:
                extra_info = {'question': extra_info, 'answer': extra_info}

        raw_question = row['question']
        if self.first_prompt == 'raw':
            prompt = row['question']  # raw question
        elif self.first_prompt == 'train':
            prompt = extra_info['question']  # prompt with question
        else:
            raise NotImplementedError(f"Unknown prompt type {self.first_prompt}")
        ...
        attention_mask = attention_mask.unsqueeze(0)
        float_attention_mask = torch.full_like(attention_mask, -torch.inf, dtype=torch.bfloat16)
        float_attention_mask = float_attention_mask.masked_fill(attention_mask, 0.0)
        ...
        if prompt_ids.shape[0] > 1:
            loss_mask[: min(prompt_ids.shape[0], loss_mask.size(0)) - 1] = 0
        loss_mask[min(prompt_ids.shape[0] + response_ids.shape[0], loss_mask.size(0)) - 1] = 0
        ...
        return {
            ...
            "position_ids": position_ids,
            "loss_mask": loss_mask,
            "raw_prompt": prompt_chat_str,  # Formatted prompt (string)
            "raw_response": response,  # Raw answer (string)
            "raw_problem": raw_question,  # Raw prompt (string)
        }
