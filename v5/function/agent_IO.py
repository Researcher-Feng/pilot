...
from v5.prompt.system import *
from v5.function.contex_fun import Context
from v5.function.format_fun import ResponseFormat
from v5.function.memory_fun import simple_checkpointer
from config_secret.api_config import ds_key_url, key_config, key_url, smith_key
from v5.utils.evaluator import extract_answer, logger
from v5.utils.MARIO_EVAL.demo import is_equiv_MATH
...
class SimpleLocalAgent:
    ...
    def invoke(self, input_dict, config=None, context=None):
        ...
        # 记录对话历史
        self.dialogue_history.append(("user", user_input))  # dddd
...
class SimpleAgent(object):
    ...
    def _build_full_prompt(self, user_input, memory_context):  # dddd
        """构建完整提示词（增强版）"""
    ...
    def multi_agent_chat_explicit(self, teacher_agent, problem: str, raw_problem: str, correct_answer: str,
        ...
        try:
            # 学生首次尝试
            student_response_obj = self._invoke_agent('')  # dddd
            # 确保正确提取响应文本
            ...
            for turn in range(self.max_turns):
                ...
                if self.parallel_thinking_enabled and turn == 0:
                    ...
                    teacher_input = self._format_teacher_input(problem, self.dialogue_history)  # dddd
                    if dialogue_record.debug_mode:
                        logger.info(f"👨‍🏫 teacher_input [轮次{current_turn}]: {teacher_input}")
                    # teacher_agent.dialogue_history = self.dialogue_history  # dddd
                ...
                # student_input = self._format_student_input(raw_problem, self.dialogue_history)  # dddd
                if dialogue_record.debug_mode:
    ...
    def _invoke_local_with_memory(self, input_text):
        ...
        # 使用增强的完整提示词构建方法
        full_input = self._build_full_prompt(input_text, memory_context)  # dddd
    ...
    def _format_teacher_input(self, problem, history):
        ...
        if self.socratic_teaching_enabled:
            ...
            context += "Please provide appropriate guidance briefly to help the student solve the problem."
